...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar  2 18:58:32 2018
...
def TwitchBasedMuscleModel(trialN,target_amplitude,gainMatrix,control_type):
...
#        return exp(x)
    ...
    output = {'Time':time_sim,'Tendon Force':ForceSE_vec, 'Muscle Force':Force_vec, 
              ...
              'FR': FR_mat, 'f_env': f_env_mat, 'force_half': force_half_mat,
              'S': S_vec_store, 'Y': Y_vec_store,
              'Af': Af_mat, 'FF': FF_mat,
...
gainMatrix = [50,50,-0.5,-0.5,-0.5,0,0,0.1];
control_type = 1;

(Force_vec,ForceSE_vec) = TwitchBasedMuscleModel(trialN,target_amplitude,gainMatrix,control_type)
