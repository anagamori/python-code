...
CoV_ISI = np.zeros(10);
Std_ISI = np.zeros(10);
Pxx_all = np.zeros((10,100001)); # 100001 for 40000 Hz, 2501 for 1000 Hz
Force_all = np.zeros((10,5*Fs));
fileName_mean_force = "%s%s%s" % ('mean_force_0_1_0_10_',str(force_level),'.npy')  
fileName_CoV = "%s%s%s" % ('CoV_0_1_0_10_',str(force_level),'.npy')  
fileName_Std = "%s%s%s" % ('std_0_1_0_10_',str(force_level),'.npy')  
fileName_mean_ISI = "%s%s%s" % ('mean_force_0_1_0_10_',str(force_level),'.npy')  
fileName_Std_ISI = "%s%s%s" % ('std_ISI_0_1_0_10_',str(force_level),'.npy')  
fileName_CoV_ISI = "%s%s%s" % ('CoV_ISI_0_1_0_10_',str(force_level),'.npy')  
fileName_Force = "%s%s%s" % ('Force_0_1_0_10_',str(force_level),'.mat')
trials = 0;
for i in range(force_level-10,force_level):    
#for i in range(30,31):    
    
    trialN = i;
    fileName = "%s%s%s" % ('output_SDN_0_1_0_10_',str(trialN),'.npy')   
    os.chdir(save_path)
